...
/* specifies the fully qualified path to the sentiment model .astore file to upload for use in scoring. You must store the sentiment model .astore file from the downloaded score code in a location that is network-accessible by the CAS server. You must modify the value of local_astore_file_path to provide the path to the .astore file, such as "/vta/scoring/{sentiment model.astore file}". */
%let local_astore_file_path = "{local_astore_file_path}/{sentiment model.astore file}";

/* After uploading the sentiment model .astore file, you must specify a CAS library to write out the astore table to use in scoring. This library will be used in the CAS session during the ASTORE scoring action. You must modify the value to provide the name of the library that will contain the astore table. */
%let input_astore_caslib_name = "{input_astore_caslib}";

/* specifies the CAS astore table to use in scoring */
%let input_astore_name = "Sentiment_Astore";
...
%let output_table_name = "out_sentiment_astore";
