...
for i, row in input_df.iterrows():
    rowResult.append(llm_guardrails_ollama.scoreModel(row['prompt'],row['system'],row['history'],row['model'],row['embedder'],row['collection'],row['host'],row['model_port'],row['vector_port']))
output_df = pd.DataFrame(rowResult, columns=['response'])
